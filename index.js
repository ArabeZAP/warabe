...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Slv @${num.split('@')[0]}\nBv ao grupo *${mdata.subject}* não seja ghost e pague uma coquinha pro árabe`
				...
				teks = `ala nao aguentou kk @${num.split('@')[0]} vai nem fazer falta ghost do carai👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Da uma sentada enquanto carrega',
				success: 'pronto garai',
                                levelon: '❬ ✔ ❭ *habilitar Level*',
				leveloff: ' ❬ X ❭  *desabilitar Level*',
				levelnoton: '❬ X ❭ *level não ativo*',
				levelnol: '*Pqp kskst level* 0 ',
				error: {
					stick: 'Deu erro ai mano,na próxima faz certo fvr',
					Iv: 'Link inválido'
				},
				only: {
					group: '🤬Este comando é pra grupo, kkkkk burrao',
					ownerG: '🤬Este comando só pode ser usado pelo dono do grupo',
					ownerB: '🤬Este comando só pode ser usado pelo Árabe',
					admin: '😂Ala achando q é adm kkkkkkkk vsf',
					Badmin: '🥵Tem q me dar adm burro',
                                        daftarB: `── 「REGISTRE-SE」 ──\nslv mano \nVocê n está registrado no banco de dados Do BOT, tem que se cadastrar pra ter acesso aos meus comandos, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar arab|19`,
				}
			}
			...
			const ownerNumber = ["5515998130097@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5515998130097@s.whatsapp.net'
                        /******ApiKey Input******/
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`*「mais um desocupado subiu de nível」*\n\n➸ *Nome*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\n parabéns por não ser ghost 🥰`)
                }
            } catch (err) {
                console.error(err)
            }
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Só adms pode clonar carai')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `n mamou direito perdeu adm otario kkkkkk:\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`n mamou direito perdeu adm otario kkkkkk @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marca o baiano q vai ganhar adm')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'esse mamou direitinho e ganhou adm fml sem escândalo kkkkk:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`esse mamou direitinho e ganhou adm fml sem escândalo kkkkkkk : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui está o número do meu dono,',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite arabe|pauzin`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo arabe pauzin`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Título* : ${anu.title}\n\nJa encontrei, agora vou te mandar 🥰`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `aki esta os cornos do grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('Acorda seus bainos 🤡\n╠➥'+teks+'dorme  15h por dia vsf kkkkkkkkkk', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO TODO MUNDO QUEM NN GOSTA QUE SE FODA🤬💪 〙✪══\n╠➥'+teks+'╚═〘 achou ruim vaza kkkm 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO TODO MUNDO QUEM NN GOSTA QUE SE FODA🤬💪 〙✪══\n╠➥'+teks+'╚═〘 achou ruim vaza kkkkk 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'ok vou sair só não me bota mais nessa merda fdp 😙', text) // ur cods
					}, 0)
                     break
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O dono do grupo é esse corno : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('quem vc quer colocar carai?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar o meliante, talvez porque é privado')
					}
					break
				...
				case 'ban':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('tem q marcar o baiano né')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Removi memo fodase kkk :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Removi memo fodase kkk : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\no cara nem deixou eu dormir slk`,
					contextInfo: { mentionedJid: [sender] }
					}
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `Achei essa aqui!\nTítulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\nCalma ai, o vizinho desligo o wifi`
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply('1 ativa')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('ja ta ativado burro ')
						welkom.push(from)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('mds agora vou ter q falar bem vindo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('aleluia n aguentava mais fdp')
					} else {
            ...
            break
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('ve se deu certo ai')
					...
					try {
						...
						mentions(`Foto clonada com sucesso agora to usando essa bosta🤬 @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('ele ta sem foto')
					}
					break
                                /*case 'magernulis':
                                        var gh = body.slice(11)
